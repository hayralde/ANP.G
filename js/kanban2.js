const COLUMNS = ["A DEFINIR", "PARADO", "EM ANDAMENTO", "CONCLUIDO"];
let activities = [];
let dragId = null;
...
function normalizeStatus(s) {
  ...
  if (COLUMNS.indexOf(u) !== -1) return u;
...
function render() {
  var list = filtered();
  var counts = { "A DEFINIR": 0, "PARADO": 0, "EM ANDAMENTO": 0, "CONCLUIDO": 0 };
  list.forEach(function (a) {
  ...
  COLUMNS.forEach(function (status) {
    ...
    var items = list.filter(function (a) { return normalizeStatus(a.status) === status; });
    if (!items.length) {
...
function cardHtml(a) {
  var pend = a.pendencia && a.pendencia !== "A DEFINIR";
  return (
    '<article class="kb-card" draggable="true" data-id="' + a.id + '">' +
      ...
      '<div class="kb-card-meta">' +
        ...
        (hasPrazo(a.previsao) ? '<span class="kb-chip date">' + esc(a.previsao) + "</span>" : "") +
      "</div>" +
      '<div class="kb-card-foot">' +
        '<span class="kb-card-id">#' + a.id + "</span>" +
...
async function moveCard(id, newStatus) {
  ...
  if (normalizeStatus(prev) === newStatus) return;
  ...
  try {
    activities = await saveActivities2ToApi(activities);
    setStatusMsg("Status atualizado: " + newStatus, "ok");
  } catch (err) {
